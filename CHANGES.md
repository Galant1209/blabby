...
- Added `profiles.reading_band_updated_at` (mirrors Speaking's band tracking)
- Reading vocab saves now enrich with Traditional Chinese translation via new `/vocab/translate_zh` endpoint (LRU-cached, +~$0.001 per uncached save)
- E2E Pro-bypass test now uses a dedicated permanent-Pro test user instead of mutating the primary test user — no more prod data drift from test runs
- Hub label finalised: `Reading` (single word, mirrors Speaking)

Known deferred items:
- Pro soft cap (no daily limit on Pro Reading attempts) — revisit if cost monitoring shows abuse
- Reading vocab cards render in lexicon with the same layout as Speaking cards (acceptable since translation is now present)
